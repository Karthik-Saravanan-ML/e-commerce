...
import { Upload, BookOpen, TrendingUp, Star, DollarSign, AlertCircle, Plus, Edit2, Trash2, CheckCircle, XCircle, Clock } from 'lucide-react';
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Welcome, {user?.name} ✍️</h1>
...
export function UploadBook() {
  ...
  const [form, setForm] = useState({ title: '', description: '', category: 'fiction', price: '', mrp: '', stock: '0', language: 'English', tags: '', isbn: '', publisher: '', publishedYear: '', pages: '', isPhysical: true, isEbook: false, isAudiobook: false });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (pdfFile) {
        ...
        await api.post(`/books/${bookId}/upload-pdf`, pdfFd, { headers: { 'Content-Type': 'multipart/form-data' } });
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-sm font-medium text-gray-700 block mb-1">Stock</label>
              <input type="number" className="input" value={form.stock} onChange={e => set('stock', e.target.value)} /></div>
        ...
        <div className="card p-5">
          ...
          <div className="flex gap-4 flex-wrap">
            {[['isPhysical', '📦 Physical Book'], ['isEbook', '📱 eBook'], ['isAudiobook', '🎧 Audiobook']].map(([key, label]) => (
          ...
          {form.isEbook && (
            <div className="mt-4">
              <label className="text-sm font-medium text-gray-700 block mb-2">Upload PDF (eBook)</label>
              <input type="file" accept=".pdf" onChange={e => setPdfFile(e.target.files[0])}
                className="block w-full text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-primary-100 file:text-primary-700 hover:file:bg-primary-200 cursor-pointer" />
