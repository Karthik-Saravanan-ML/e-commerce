...
export const initiateRazorpayPayment = async ({ orderId, amount, currency = 'INR', name, description, onSuccess, onFailure }) => {
  try {
    ...
    const options = {
      ...
      theme: { color: '#6366f1' },
      modal: { ondismiss: () => { toast('Payment cancelled', { icon: 'ℹ️' }); onFailure && onFailure(); } },
    };
...
export const initiateSubscriptionPayment = async ({ subscriptionType, planType, userName, onSuccess }) => {
  try {
    ...
    toast.error('Could not initiate subscription');
  }
};
...
export const CATEGORIES = [
  ...
  { value: 'test-prep', label: 'Test Prep', emoji: '✏️' },
...
export const getCategoryEmoji = (cat) => CATEGORIES.find(c => c.value === cat)?.emoji || '📦';
