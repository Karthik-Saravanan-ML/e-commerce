...
import { BookOpen, Users, ShoppingBag, DollarSign, TrendingUp, CheckCircle, XCircle, Clock, Eye, Trash2, ToggleLeft, ToggleRight, Plus, Image, MapPin, Phone, Star, Sparkles, Ticket } from 'lucide-react';
import api from '../../utils/api';
import { formatPrice, formatDate, ORDER_STATUSES } from '../../utils/helpers';
import LoadingSpinner from '../../components/common/LoadingSpinner';
import toast from 'react-hot-toast';
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, LineChart, Line, CartesianGrid, Area, AreaChart, Legend } from 'recharts';

const MONTH_NAMES = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

const formatChartMonth = (entry) => {
  const month = entry?._id?.month;
  const year = entry?._id?.year;
  if (!month || !year) return '';
  return `${MONTH_NAMES[month - 1] || month} '${String(year).slice(-2)}`;
};

const buildChartData = (rows = []) =>
  rows.map((d) => ({
    ...d,
    label: formatChartMonth(d),
    revenue: d.revenue || 0,
    orders: d.orders || 0,
  }));
...
const RevenueTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-slate-900 text-white rounded-xl px-4 py-3 shadow-xl border border-slate-700">
      <p className="text-xs text-slate-400 mb-1">{label}</p>
      <p className="font-bold text-lg text-indigo-300">{formatPrice(row?.revenue || 0)}</p>
      <p className="text-xs text-slate-400 mt-1">{row?.orders || 0} orders</p>
    </div>
  );
};
...
export function AdminDashboard() {
  ...
  const chartData = buildChartData(data?.revenueByMonth);
  const periodRevenue = chartData.reduce((sum, m) => sum + m.revenue, 0);
  const periodOrders = chartData.reduce((sum, m) => sum + m.orders, 0);
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {[
          ...
          <div key={label} className="card p-4">
            <div className={`w-9 h-9 ${KPI_STYLES[color]?.bg || 'bg-gray-100'} rounded-xl flex items-center justify-center mb-3`}>
              <Icon className={`w-5 h-5 ${KPI_STYLES[color]?.text || 'text-gray-600'}`} />
            </div>
      ...
      {/* Revenue chart */}
      <div className="card p-6 mb-6 bg-gradient-to-br from-indigo-50 via-white to-violet-50 border border-indigo-100 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="font-semibold text-gray-900 text-lg">Revenue Trend</h2>
            <p className="text-sm text-gray-500 mt-1">Last 6 months · paid orders only</p>
          </div>
          <div className="flex items-center gap-2 text-indigo-600 bg-indigo-100 px-3 py-1.5 rounded-full text-sm font-medium">
            <TrendingUp className="w-4 h-4" />
            {chartData.length ? formatPrice(periodRevenue) : '₹0'}
          </div>
        </div>
        {chartData.length > 0 ? (
          <>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
                ...
                <YAxis
                  ...
                  width={55}
                />
                <YAxis yAxisId="orders" orientation="right" tick={{ fontSize: 11, fill: '#10b981' }} axisLine={false} tickLine={false} width={30} />
                <Tooltip content={<RevenueTooltip />} cursor={{ stroke: '#6366f1', strokeWidth: 1, strokeDasharray: '4 4' }} />
                <Legend verticalAlign="top" height={28} iconType="circle" />
                <Area yAxisId="revenue" type="monotone" dataKey="revenue" stroke="none" fill="url(#colorRevenue)" legendType="none" />
                <Line
                  yAxisId="revenue"
                  type="monotone"
                  dataKey="revenue"
                  ...
                  activeDot={{ r: 7, fill: '#4338ca', stroke: '#fff', strokeWidth: 2 }}
                  name="Revenue"
                />
                <Line
                  yAxisId="orders"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-indigo-100">
              <div className="bg-white/70 rounded-xl p-3 border border-indigo-50">
                <p className="text-xs text-gray-500">Period Revenue</p>
                <p className="font-bold text-lg text-indigo-600">{formatPrice(periodRevenue)}</p>
              </div>
              <div className="bg-white/70 rounded-xl p-3 border border-emerald-50">
                <p className="text-xs text-gray-500">Period Orders</p>
                <p className="font-bold text-lg text-emerald-600">{periodOrders}</p>
              </div>
              <div className="bg-white/70 rounded-xl p-3 border border-indigo-50">
                <p className="text-xs text-gray-500">Avg / Month</p>
                <p className="font-bold text-lg text-indigo-600">{formatPrice(periodRevenue / (chartData.length || 1))}</p>
              </div>
              <div className="bg-white/70 rounded-xl p-3 border border-violet-50">
                <p className="text-xs text-gray-500">Months Tracked</p>
                <p className="font-bold text-lg text-violet-600">{chartData.length}</p>
              </div>
            </div>
          </>
        ) : (
          <div className="h-48 flex items-center justify-center text-gray-400 text-sm bg-white/50 rounded-xl border border-dashed border-gray-200">
            No revenue data yet — orders will appear here once payments are confirmed.
          </div>
        )}
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => toggleBestSeller(book._id)} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg ${book.isBestSeller ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                    <Star className="w-3 h-3" /> {book.isBestSeller ? 'Best Seller' : 'Set Best Seller'}
                  </button>
                  <button onClick={() => toggleFeatured(book._id)} className={`flex items-center gap-1 text-xs px-3 py-1.5 rounded-lg ${book.isFeatured ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                    <Sparkles className="w-3 h-3" /> {book.isFeatured ? 'Featured' : 'Set Featured'}
                  </button>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {addr && (
                      <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                        ...
                        <p className="text-sm font-semibold text-gray-900">{addr.name || order.user?.name}</p>
                        {addr.street && <p className="text-sm text-gray-700 mt-0.5">{addr.street}</p>}
...
export function AdminBanners() {
  ...
  const load = () => {
    api.get('/admin/banners').then(r => setBanners(r.data.banners || [])).catch(() => {
      api.get('/banners?all=true').then(r => setBanners(r.data.banners || []));
    }).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to create banner. Log in as admin to upload banners.');
    }
                  ...
                  <div>
                    ...
                    {banner.couponCode && (
                      <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                        <Ticket className="w-3 h-3" /> {banner.couponCode} — {banner.discountPercent}% off
                      </p>
                    )}
                  </div>
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Revenue', value: formatPrice(data?.stats?.totalRevenue || 0), icon: DollarSign, bgClass: 'bg-gradient-to-br from-green-50 to-green-100 border-green-200' },
          { label: 'Total Orders', value: data?.stats?.totalOrders || 0, icon: ShoppingBag, bgClass: 'bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200' },
          { label: 'Total Users', value: data?.stats?.totalUsers || 0, icon: Users, bgClass: 'bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200' },
          { label: 'Total Books', value: data?.stats?.totalBooks || 0, icon: BookOpen, bgClass: 'bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200' },
        ].map(({ label, value, icon: Icon, bgClass }) => (
          <div key={label} className={`card p-5 ${bgClass}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        {/* Revenue Chart */}
        <div className="card p-6 bg-gradient-to-br from-slate-50 to-slate-100 border-indigo-100">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data?.revenueByMonth}>
              ...
              <XAxis dataKey={d => `${d._id.month}/${String(d._id.year).slice(2)}`} tick={{ fontSize: 12, fill: '#64748b' }} />
              ...
              />
              <Bar dataKey="revenue" fill="url(#colorBar)" radius={[6, 6, 0, 0]} name="Revenue" />
            </BarChart>
          </ResponsiveContainer>
        </div>

        {/* Orders Chart */}
        <div className="card p-6 bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-100">
          <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
      ...
      {/* Summary Stats */}
      <div className="card p-6 bg-gradient-to-r from-slate-50 via-blue-50 to-slate-50">
        <h2 className="font-semibold text-gray-900 mb-6 text-lg">6-Month Summary</h2>
        <div className="grid grid-cols-3 gap-6">
          <div className="border-l-4 border-indigo-600 pl-4">
            <p className="text-sm text-gray-600 mb-1">Total Revenue Period</p>
            <p className="text-2xl font-bold text-indigo-600">{formatPrice(data?.revenueByMonth?.reduce((sum, m) => sum + m.revenue, 0) || 0)}</p>
          </div>
          <div className="border-l-4 border-emerald-600 pl-4">
            <p className="text-sm text-gray-600 mb-1">Total Orders Period</p>
            <p className="text-2xl font-bold text-emerald-600">{data?.revenueByMonth?.reduce((sum, m) => sum + m.orders, 0) || 0}</p>
          </div>
          <div className="border-l-4 border-purple-600 pl-4">
            <p className="text-sm text-gray-600 mb-1">Average Revenue/Month</p>
            <p className="text-2xl font-bold text-purple-600">{formatPrice((data?.revenueByMonth?.reduce((sum, m) => sum + m.revenue, 0) || 0) / (data?.revenueByMonth?.length || 1))}</p>
          </div>
        </div>
      </div>
    </div>
