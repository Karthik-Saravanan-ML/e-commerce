...
import { MapPin, CreditCard, Truck, Banknote, Package } from 'lucide-react';
...
export default function Checkout() {
  ...
  return (
    <div className="page-container-narrow py-8 sm:py-10">
      <PageHeader title="Checkout" subtitle="Review your order and complete payment" breadcrumbs={[{ to: '/cart', label: 'Cart' }, { label: 'Checkout' }]} />
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { val: 'online', label: 'Online Payment', desc: 'UPI, Cards, Net Banking via Razorpay', icon: CreditCard },
        ...
        <div className="card p-6 h-fit sticky top-24 space-y-4">
          ...
          <button onClick={placeOrder} disabled={loading} className="btn-primary w-full py-3 text-base">
            {loading ? 'Placing Order…' : paymentMethod === 'cod' ? (
              <span className="flex items-center justify-center gap-2"><Package className="w-4 h-4" /> Place Order (COD)</span>
            ) : (
              <span className="flex items-center justify-center gap-2"><CreditCard className="w-4 h-4" /> Pay & Place Order</span>
            )}
          </button>
        </div>
      </div>
