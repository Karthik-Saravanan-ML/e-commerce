...
export function MyOrders() {
  ...
  return (
    <div className="page-container-tight py-8 sm:py-10">
...
export function Wishlist() {
  ...
  return (
    <div className="page-container py-8 sm:py-10">
      <PageHeader title="My Wishlist" subtitle={`${filtered.length} book${filtered.length !== 1 ? 's' : ''} saved`} />

      {books.length === 0 ? (
        <div className="text-center py-20">
          <Heart className="w-16 h-16 mx-auto mb-4 opacity-20 text-gray-400" />
          ...
          <Link to="/shop" className="btn-primary inline-block px-8 py-3">Discover Books</Link>
        </div>
      ) : (
        <>
            ...
            {filtered.map(book => (
              <div key={book._id} className="group relative">
                ...
                <button onClick={() => remove(book._id)}
                  className="absolute top-2 right-2 w-9 h-9 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition z-10 shadow-lg opacity-0 group-hover:opacity-100">
          ...
          {filtered.length === 0 && books.length > 0 && (
            <div className="text-center py-12 text-gray-400">
              <p>No books match your filters. Try adjusting them.</p>
            </div>
          )}
        </>
      )}
  ...
  return (
    <div className="page-container py-8 sm:py-10">
      <PageHeader
        ...
        breadcrumbs={[{ to: '/', label: 'Home' }, { to: '/shop', label: 'Shop' }, { label: catInfo?.label || category }]}
      >
        <select className="input w-full sm:w-44" value={sort} onChange={e => { setSort(e.target.value); setPage(1); }}>
