...
import { Search, UserPlus, MessageCircle, Globe, Send, Check, Users, UserCheck, Clock, Image, Mic, Paperclip, X, Languages, Lightbulb } from 'lucide-react';
...
export function WriterNetwork() {
  ...
  const loadRequests = async () => {
    try {
      const { data } = await api.get('/writers/friend-requests');
  ...
  const searchWriters = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`/writers/search?q=${encodeURIComponent(search)}`);
  ...
  useEffect(() => {
    ...
    socket.on('friend_request_accepted', onFriendRequest);
    return () => {
      socket.off('friend_request', onFriendRequest);
      socket.off('friend_request_accepted', onFriendRequest);
    };
  }, []);
  ...
  const sendRequest = async (writerId) => {
    ...
    try {
      ...
      setWriters(prev => prev.map(w => w._id === writerId ? { ...w, connectionStatus: 'pending_sent' } : w));
    } catch (err) {
  ...
  const respondRequest = async (requestId, action) => {
    try {
      await api.put(`/writers/friend-request/${requestId}`, { action });
  ...
  const startChat = async (writerId) => {
    try {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-gray-500 text-sm mb-6">Connect with authors worldwide. Messages auto-translate to your language.</p>

      {friendRequests.length > 0 && (
        <div className="card p-5 mb-6 border-primary-100 bg-primary-50/40">
          ...
          <div className="space-y-3">
            {friendRequests.map((req) => (
              <div key={req._id} className="flex items-center justify-between gap-3 bg-white rounded-xl p-3 border border-gray-100">
                <div className="flex items-center gap-3 min-w-0">
                  <img src={req.from?.avatar || `https://ui-avatars.com/api/?name=${req.from?.name}&background=4f46e5&color=fff`}
                    alt="" className="w-10 h-10 rounded-full flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="font-medium text-sm text-gray-900 truncate">{req.from?.name}</p>
                    <p className="text-xs text-gray-500">Wants to connect with you</p>
                  </div>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => respondRequest(req._id, 'accept')}
                    className="text-xs px-3 py-1.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 flex items-center gap-1">
                    <Check className="w-3 h-3" /> Accept
                  </button>
                  <button onClick={() => respondRequest(req._id, 'reject')}
...
function ConnectButton({ status, writerId, onConnect, sending }) {
  if (status === 'friends') {
    return (
      <span className="flex-1 text-xs py-2 border border-green-200 bg-green-50 text-green-700 rounded-lg flex items-center justify-center gap-1">
  ...
  if (status === 'pending_sent') {
    return (
      <span className="flex-1 text-xs py-2 border border-amber-200 bg-amber-50 text-amber-700 rounded-lg flex items-center justify-center gap-1">
  ...
  if (status === 'pending_received') {
    return (
      <span className="flex-1 text-xs py-2 border border-indigo-200 bg-indigo-50 text-indigo-700 rounded-lg flex items-center justify-center gap-1">
        <UserPlus className="w-3 h-3" /> Respond above
      </span>
    );
  }
  return (
    <button onClick={() => onConnect(writerId)} disabled={sending[writerId]}
  ...
  return (
    <div className="card p-5 hover:shadow-md transition-all">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {writer.writerProfile?.language && (
            <span className="badge bg-indigo-100 text-indigo-700 text-xs mt-1">
              <Languages className="w-3 h-3" /> {writer.writerProfile.language}
            </span>
          )}
      ...
      <div className="flex gap-2 mt-4">
        <ConnectButton status={writer.connectionStatus} writerId={writer._id} onConnect={onConnect} sending={sending} />
        <button onClick={() => onChat(writer._id)}
...
const isMediaMessage = (msg) => ['image', 'video', 'audio'].includes(msg?.messageType);
...
export function WriterChat() {
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      const onMsg = (msg) => {
        if (msg.conversationId === activeConv) setMessages((prev) => appendUniqueMessage(prev, msg));
      };
  ...
  const uploadMedia = async (file) => {
    ...
    try {
      ...
      await api.post(`/writers/conversation/${activeConv}/media`, fd, {
  ...
  const startVoiceRecord = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (!chunks.length) {
          toast.error('Recording too short — hold mic longer');
          return;
        }
  ...
  const renderMessageBody = (msg) => {
    if (msg.messageType === 'image' && msg.fileUrl) {
      return <img src={msg.fileUrl} alt="" className="max-w-full rounded-xl max-h-52 object-cover shadow-sm" />;
    }
    if (msg.messageType === 'video' && msg.fileUrl) {
      return <video src={msg.fileUrl} controls className="max-w-full rounded-xl max-h-52 shadow-sm" />;
    }
    if (msg.messageType === 'audio' && msg.fileUrl) {
      return (
        <div className="bg-white rounded-xl p-2 shadow-sm border border-gray-100 min-w-[220px]">
          <audio src={msg.fileUrl} controls preload="metadata" className="w-full h-10" />
          <p className="text-[10px] text-gray-400 mt-1 px-1">Voice message</p>
        </div>
      );
    }
    return (
      <>
        ...
        {msg.displayText && msg.originalText && msg.displayText !== msg.originalText && (
        ...
        {!activeConv ? (
          <div className="flex-1 flex items-center justify-center text-gray-400">
            <div className="text-center">
              ...
              <p className="text-sm mt-1 text-indigo-400 flex items-center justify-center gap-1.5">
                <Lightbulb className="w-4 h-4" /> Messages auto-translate to each writer&apos;s language
              </p>
            </div>
          </div>
        ) : (
              ...
              return other ? (
                <div className="bg-white border-b border-gray-200 px-5 py-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-indigo-500 flex items-center gap-2 flex-wrap">
                      <span className="inline-flex items-center gap-1"><Globe className="w-3 h-3" /> Auto-translate</span>
                      <span className="inline-flex items-center gap-1"><Image className="w-3 h-3" /> Photos</span>
                      <span className="inline-flex items-center gap-1"><Mic className="w-3 h-3" /> Voice</span>
                    </p>
                  </div>
                </div>
              ) : null;
                  ...
                  <div key={msg._id || i} className={`flex items-end gap-2 ${isOwn ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={
                      isMedia
                        ...
                        : `max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl text-sm ${isOwn ? 'bg-primary-600 text-white rounded-br-md' : 'bg-white text-gray-800 shadow-sm rounded-bl-md'}`
                    }>
            ...
            <form onSubmit={sendMessage} className="bg-white border-t border-gray-200 p-3">
              ...
              <div className="flex gap-2 items-end">
                <button type="button" onClick={() => fileInputRef.current?.click()}
                  className="p-2.5 text-gray-500 hover:bg-gray-100 rounded-xl" title="Photo or video">
                  <Image className="w-5 h-5" />
                </button>
                <button type="button" onClick={recording ? stopVoiceRecord : startVoiceRecord}
                  className={`p-2.5 rounded-xl ${recording ? 'bg-red-100 text-red-600' : 'text-gray-500 hover:bg-gray-100'}`} title="Voice message">
                  {recording ? <X className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
                </button>
                <input className="input flex-1 text-sm min-h-[44px]" placeholder="Type a message…"
                  value={newMessage} onChange={handleTyping} />
                <button type="submit" disabled={sending || !newMessage.trim()}
                  className="btn-primary p-3 aspect-square flex items-center justify-center disabled:opacity-50">
